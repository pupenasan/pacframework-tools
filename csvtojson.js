...
fs.readdir(directoryPath, (err, files) => {
  ...
  files.forEach((file) => {
    if (path.extname(file) === '.csv') {
      const filename = `./cm/${path.basename(file, '.csv')}`;
      csvpath.push(filename);
    }
  });
  for (const path1 of csvpath) {
    ...
    csv({ delimiter: '|', ignoreEmpty: true })
      ...
      // console.log(jsonObj);
        for (const rec of jsonObj) {
          if (rec.name[0] !== '-') {
            target[filename][rec.name] = {
              type: rec.type, adr: Number.parseInt(rec.adr), bit: Number.parseInt(rec.bit) || 0, descr: rec.descr || rec.name,
            };
          }
        // console.log(target);
        }
