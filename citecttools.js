...
function getequipmenttable(connstr, idx) {
  ...
  let records = runsql (connstr, sqlcmd, idx);
  return records
}
...
function modifyequipments(newequipments, newequiprtpara) { 
  ...
  for (let tagprefix in newequipments){
    ...
    if (oldequipments[tagprefix]) {
      ...
      sqlcmd = sqlcmd.slice(0, -1);//убрати останню кому
      sqlcmd += ` WHERE tagprefix='${tagprefix}'; `;
      if (sqlcmd.length>0) runsql (connstrprj, sqlcmd);
    } else {
      let sqlcmdadd = '';
      let fieldnames = '', vals = '';
      ...
      //console.log (sqlcmdadd);
      sqlcmdadd += `INSERT INTO equip VALUES (${vals.slice(0, -1)}); `;
      if (sqlcmdadd.length>0) runsql (connstrprj, sqlcmdadd)
    }
    ...
    if (newequiprtpara[rtparavalue] && oldequiprtpara[rtparavalue] && newequiprtpara[rtparavalue].equip && oldequiprtpara[rtparavalue].equip !== newequiprtpara[rtparavalue].equip){
      ...
      sqlcmdadd += `INSERT INTO eqparam VALUES (${vals.slice(0, -1)}); `;
      //console.log (sqlcmdadd);
      if (sqlcmdadd.length>0) runsql (connstrprj, sqlcmdadd)
    } 
  }
...
function runsql (connstr, sqlcmd, idx){
  let paras;
  paras = ['dbasetools.vbs', 'runsql', connstr, sqlcmd];
  ...
  str = iconv.decode(Buffer.from(vbs.stdout), 'win1251');
  ...
  if (records.type==='TABLE') tabtoob (records, str, idx); 
  return records  
}
...
function getequipmenttypesinfo (projectpath){
  ...
  for (let filename of filenames) {
    ...
    if (ext === '.xml') { 
      ...
      try {
        xmlcontent = xmlparser.xml2js(content, {
          compact: true,
          spaces: 4,
        });
        if (xmlcontent.template && xmlcontent.template.input && xmlcontent.template.input._attributes && xmlcontent.template.input._attributes.name === 'equipment') {
          ...
          for (let atribute of xmlcontent.template.param.string){
            if (atribute._attributes.name === 'name') equipment = equiptypes[atribute._text] = {name: atribute._text};
            if (atribute._attributes.name === 'parameter-definitions') {
