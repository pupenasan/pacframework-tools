...
function tiaparseall() {
  ...
  let listfiles = {
    ...
    xmlcfgfile: config.tiaparsetools.plc_xmlcfgfile
      .replace(/ /g, '')
      .split(','),
  };
  ...
  listfiles = {
    ...
    xmlcfgfiles: config.tiaparsetools.ch_xmlcfgfiles
      .replace(/ /g, '')
      .split(','),
    xmlhmifiles: config.tiaparsetools.ch_xmlhmifiles
      .replace(/ /g, '')
      .split(','),
  ...
  logmsg(
    '-------------------- Отримання даних про відображення каналів на модулі',
  );
  ...
  listfiles = {
    ...
    xmlcfgfiles: config.tiaparsetools.var_xmlcfgfiles
      .replace(/ /g, '')
      .split(','),
    xmlhmifiles: config.tiaparsetools.var_xmlhmifiles
      .replace(/ /g, '')
      .split(','),
  ...
  logmsg(
    '-------------------- Формування таблиці відображення тегів на канали',
  );
  ...
  fs.writeFileSync(
    `${tiaresultfiles}plc_chs.json`,
    JSON.stringify(plchs),
    'utf8',
  );
  fs.writeFileSync(
    `${tiaresultfiles}plc_tags.json`,
    JSON.stringify(plctags),
    'utf8',
  );
  ...
  listfiles = {
    udtfiles: [],
    xmlcfgfiles: config.tiaparsetools.act_xmlcfgfiles
      .replace(/ /g, '')
      .split(','),
    xmlhmifiles: config.tiaparsetools.act_xmlhmifiles
      .replace(/ /g, '')
      .split(','),
  ...
  fs.writeFileSync(
    `${tiaresultfiles}plc_acts.json`,
    JSON.stringify(plcacts),
    'utf8',
  );
  ...
  writetolog(1);
  return {
    plchs, plctags, plcacts, iot, plcs,
  };
}
...
function parseiotbuf(iotplc, pathfiles, listfiles) {
  ...
  const dbcmplt = parsedb(xmlcontent, parsedata);
  ...
  const parseiotbuf = dbcmplt.data.IOTBUF;
...
function plc_to_plcs(masterplcs, pathfiles, listfiles) {
  ...
  // отримання структур
  if (typeof masterplcs.types !== 'object') {
    masterplcs.types = {};
  } // пошук структур якщо вони вже є в метаданих
  const { udtfiles } = listfiles;
...
function plcacts_toacts(masteracts, pathfiles, listfiles) {
  ...
  // отримання структур
  if (typeof masteracts.types !== 'object') {
    masteracts.types = {};
  } // пошук структур якщо вони вже є в метаданих
  const { udtfiles } = listfiles;
  for (let i = 0; i < udtfiles.length; i++) {
  ...
  for (let i = 0; i < xmlcfgfiles.length; i++) {
    ...
    try {
      ...
      logmsg(
        'Помилка читання файлу, можливо файлу немає в директорії, завершую роботу ',
      );
      process.exit();
    }
    ...
    dbcmplt = parsedb(xmlcontent, parsedata);
    fs.writeFileSync(
      `${pathfiles}actcfg_parse.json`,
      JSON.stringify(dbcmplt),
      'utf8',
    );
  ...
  let fileparseresult;
  const { xmlhmifiles } = listfiles; //  ACTH;
...
function ACTCFG_to_acts(parsedata, masteracts) {
  ...
  if (typeof masteracts.acts !== 'object') {
    masteracts.acts = {};
  } // за тегами
  if (typeof masteracts.ids !== 'object') {
    masteracts.ids = {};
  } // індексація за ID
  for (actname in parsedata.data) {
    ...
    const parseact = parsedata.data[actname];
    act.actname = actname;
    ...
    const acttype = masteracts.types[act.plccfg.type]; // доступ до об'єкту-типу
    ...
    let descrorig = '';
    let res = {};
    for (const fieldname in parseact) {
      ...
      //
      const fieldtype = parseact[fieldname].type;
      // якщо є відомі структури всередині
      if (fieldtype && masteracts.types[fieldtype]) {
        // перебираємо біти
        for (const bitname in parseact[fieldname]) {
          if (typeof parseact[fieldname][bitname] !== 'object') continue;
          if (
            parseact[fieldname][bitname].descr
            && parseact[fieldname][bitname].descr.length > 2
          ) {
...
function ACTHMI_to_acts(dbcmplt, masteracts) {
  ...
  for (actname in dbcmplt.data) {
    let startadrbyte = -1; // початкова адреса структури
    let actplchmi = {};
    const parseact = dbcmplt.data[actname];
    actplchmi = parseact;
    // ------------------- обробка plchmi
    ...
    for (const fieldname in parseact) {
      ...
      // шукаємо мінімльну адресу байту зміщення в структурі
      if (
        parseact[fieldname].adr
        && typeof parseact[fieldname].adr.byte === 'number'
        && (startadrbyte < 0
          || parseInt(parseact[fieldname].adr.byte) < startadrbyte)
      ) {
    ...
    const masteracthmi = masteracts.acts[actname].plchmi;
...
function ACTBUF_to_acts(dbcmplt, masteracts) {
  ...
  const parseact = dbcmplt.data.ACTBUF;
  let sadr;
  let startadrbyte = -1; // початкова адреса структури
  // перебор полів структури
  for (const fieldname in parseact) {
...
function plcvars_to_tags(mastertags, pathfiles, listfiles) {
  ...
  // отримання структур
  if (typeof mastertags.types !== 'object') {
    mastertags.types = {};
  } // пошук структур якщо вони вже є в метаданих
  const { udtfiles } = listfiles;
  for (let i = 0; i < udtfiles.length; i++) {
  ...
  const { xmlcfgfiles } = listfiles; //  ["AIH", "DIH", "AOH", "DOH"];
  for (let i = 0; i < xmlcfgfiles.length; i++) {
    xmlcfgfiles[i] = pathfiles + xmlcfgfiles[i];
    logmsg(`Читаю файл ${`${xmlcfgfiles[i]}.xml`}`);
    xmlcontent = fs.readFileSync(`${xmlcfgfiles[i]}.xml`, 'utf8');
  ...
  let fileparseresult;
  const { xmlhmifiles } = listfiles; //  ["AIH", "DIH", "AOH", "DOH"];
  for (let i = 0; i < xmlhmifiles.length; i++) {
    xmlhmifiles[i] = pathfiles + xmlhmifiles[i];
    logmsg(`Читаю файл ${`${xmlhmifiles[i]}.xml`}`);
    xmlcontent = fs.readFileSync(`${xmlhmifiles[i]}.xml`, 'utf8');
...
function VARCFG_to_tags(dbcmplt, mastertags) {
  ...
  if (typeof mastertags.tags !== 'object') {
    mastertags.tags = {};
  } // за тегами
  ...
  for (tagname in dbcmplt.data) {
    let isrez = false;
    const parsetag = dbcmplt.data[tagname];
    const tag = {};
    tag.tagname = tagname;
    ...
    // перебор полів структури
    const tagtype = mastertags.types[tag.plccfg.type];
    let descrorig = '';
    let res = {};
    for (const fieldname in parsetag) {
      if (typeof parsetag[fieldname] !== 'object') continue;
      // зінюємо формат адреси полів до текстового
      tag.plccfg[fieldname].adr = adr_to_string(
        parsetag[fieldname].adr,
        dbcmplt.dbnum,
      );
      ...
      // поки нічого обробляти на рівні структур
      //
      const fieldtype = parsetag[fieldname].type;
      // якщо є відомі структури всередині
      if (fieldtype && mastertags.types[fieldtype]) {
        // перебираємо біти
        for (const bitname in parsetag[fieldname]) {
          // logmsg (fieldname + '-' + bitname);
          if (typeof parsetag[fieldname][bitname] !== 'object') continue;
          // зінюємо формат адреси полів до текстового
          tag.plccfg[fieldname][bitname].adr = adr_to_string(
            parsetag[fieldname][bitname].adr,
            dbcmplt.dbnum,
          );
          if (
            parsetag[fieldname][bitname].descr
            && parsetag[fieldname][bitname].descr.length > 2
          ) {
    ...
    if (typeof mastertags.tags[tagname] === 'object') {
      // шукаємо спочатку за іменем,
      mastertag = mastertags.tags[tagname];
    } else if (mastertags.ids[tag.id]) {
      // якщо не знаходимо, то шукаємо за ID
      const oldtagname = mastertags.ids[tag.id];
    ...
    const { clsid } = mastertag;
    if (!mastertags.twinsrepo[clsid]) mastertags.twinsrepo[clsid] = {};
    repoclsid = mastertags.twinsrepo[clsid];
    repoclsid[mastertag.id] = {
      ref: `tags/${mastertag.tagname}`,
      reftype: 'var',
      alias: mastertag.tagname,
    };
...
function VARHMI_to_tags(dbcmplt, mastertags) {
  ...
  for (tagname in dbcmplt.data) {
    ...
    for (const fieldname in parsetag) {
      ...
      // шукаємо мінімльну адресу байту зміщення в структурі
      if (
        typeof parsetag[fieldname].adr.byte === 'number'
        && (startadrbyte < 0
          || parseInt(parsetag[fieldname].adr.byte) < startadrbyte)
      ) {
    ...
    const mastertaghmi = mastertags.tags[tagname].plchmi;
    // синхронізація об'єктів
...
function VARBUF_to_tags(dbcmplt, mastertags) {
  ...
  const parsevar = dbcmplt.data.VARBUF;
  let sadr;
  let startadrbyte = -1; // початкова адреса структури
  // перебор полів структури
  for (const fieldname in parsevar) {
    if (typeof parsevar[fieldname] !== 'object') continue;
    // шукаємо мінімльну адресу байту зміщення в структурі
    if (
      parsevar[fieldname].adr
      && typeof parsevar[fieldname].adr.byte === 'number'
      && (startadrbyte < 0
        || parseInt(parsevar[fieldname].adr.byte) < startadrbyte)
    ) {
...
function plc_to_chs(masterchs, pathfiles, listfiles) {
  let xmlcontent; let parsedata; let
    chs;
  parsedata = {};

  // отримання структур
  if (typeof masterchs.types !== 'object') {
    masterchs.types = {};
  } // пошук структур якщо вони вже є в метаданих
  const { udtfiles } = listfiles;
  for (let i = 0; i < udtfiles.length; i++) {
  ...
  chs = masterchs.chs;
  const filename = `${pathfiles}CH.xml`;
...
function CHHMI_to_chs(parsedata, chs) {
  const CHDIs = parsedata.data.CHDI.data;
  const CHDOs = parsedata.data.CHDO.data;
  const CHAIs = parsedata.data.CHAI.data;
  const CHAOs = parsedata.data.CHAO.data;
  ...
  if (typeof chs.chdis !== 'object') {
    chs.chdis = {};
  }
  if (typeof chs.chdos !== 'object') {
    chs.chdos = {};
  }
  if (typeof chs.chais !== 'object') {
    chs.chais = {};
  }
  if (typeof chs.chaos !== 'object') {
    chs.chaos = {};
  }
  ...
  for (let i = 0; i < CHDIs.length; i++) {
    ...
    const chhmi = chs.chdis[id].plchmi;
    chhmi.type = 'CH_HMI';
  ...
  for (let i = 0; i < CHDOs.length; i++) {
    ...
    const chhmi = chs.chdos[id].plchmi;
    chhmi.type = 'CH_HMI';
...
function PLCMAPS_to_chs(masterchs, filename) {
  ...
  const arcontent = txtcontent.replace(/[\r\n\t]/g, '').split(';');
  let curmodule = '';
  let modtypestr = '';
  ...
  for (let row of arcontent) {
    ...
    if (row.search(/MODULES\[/) >= 0 && curmodule.length > 1) {
      const number = rowar[0].split('[')[1].split(']')[0];
      if (typeof iomapplc[number] !== 'object') {
        iomapplc[number] = {
          MODID: curmodule,
...
function CHBUF_to_chs(dbcmplt, masterchs) {
  ...
  const parsech = dbcmplt.data.CHBUF;
  let sadr;
  let startadrbyte = -1; // початкова адреса структури
  // перебор полів структури
  for (const fieldname in parsech) {
    if (typeof parsech[fieldname] !== 'object') continue;
    // шукаємо мінімльну адресу байту зміщення в структурі
    if (
      parsech[fieldname].adr
      && typeof parsech[fieldname].adr.byte === 'number'
      && (startadrbyte < 0 || parseInt(parsech[fieldname].adr.byte) < startadrbyte)
    ) {
...
function SUBMODULE_to_chs(dbcmplt, masterchs) {
  ...
  const submodule = dbcmplt.data.SUBMODULE;
  let sadr;
  let startadrbyte = -1; // початкова адреса структури
  // перебор полів структури
  for (const fieldname in submodule) {
    if (typeof submodule[fieldname] !== 'object') continue;
    // шукаємо мінімльну адресу байту зміщення в структурі
    if (
      submodule[fieldname].adr
      && typeof submodule[fieldname].adr.byte === 'number'
      && (startadrbyte < 0
        || parseInt(submodule[fieldname].adr.byte) < startadrbyte)
    ) {
...
function multilevel_adrparse(parsedata, dbnum) {
  ...
  if (parsedata.data && Array.isArray(parsedata.data) === true) {
    for (const fieldval of parsedata.data) {
      ...
      if (fieldval.adr) {
        ...
        fieldval.adr = sadr;
        if (
          (fieldval.data && Array.isArray(fieldval.data) === true)
          || fieldval.STA
          || fieldval.VAL
        ) {
    ...
    for (const fieldname in parsedata) {
      ...
      if (parsedata[fieldname].adr) {
        ...
        if (
          ...
          && Array.isArray(parsedata[fieldname].data) === true
...
function plcconst_to_dec(val) {
  ...
  if (ar.length > 1) {
    switch (
      parseInt(ar[0]) // система числення 16, 2
    ) {
      case 16:
        val = parseInt(ar[1], 16);
        break;
      case 2:
        val = parseInt(ar[1], 2);
        break;
...
function parsedb(xmlcontent, parsedata) {
  const docs = xmlparser.xml2js(xmlcontent, {
    compact: true,
    spaces: 4,
  }).Document;
  ...
  const ob = docs['SW.Blocks.GlobalDB'].AttributeList;
  const dbcmplt = { rowparse: ob.Interface };
  const adrob = { byte: 0, bit: 0 };
...
function parsesection(parsedata, Section, adrob) {
  ...
  if (Section.Member.length !== undefined) {
    ...
    if (adrob.bit > 0) {
      // якщо були біти в попередніх полях 11.09.21
      adrob.byte++; // зробити приріст на байт
      if (adrob.byte % 2 !== 0) {
        // якщо непаний - вирівнювання по парному
        adrob.byte++;
      }
    }
    adrob.bit = 0;
  } else {
    const memberitem = Section.Member;
    parsemember(parsedata, memberitem, adrob);
    if (adrob.bit > 0) {
      // якщо були біти в попередніх полях 11.09.21
      adrob.byte++; // зробити приріст на байт
...
function parsemember(parsedata, memberitem, adrob) {
  ...
  if (memberitem.Sections) {
    // якщо має гілки
    let filedtype = memberitem._attributes.Datatype;
    const fieldname = memberitem._attributes.Name;
    const ob = (parsedata[fieldname] = { type: filedtype.replace(/"/g, '') });
    ...
    if (filedtype.search('array') != -1) {
      filedtype = filedtype
        ...
        .replace('..', ' ')
        .replace(/"/g, '');
      const typear = filedtype.split(' ');
      const strti = typear[0];
      const endi = typear[1];
      const typei = typear[3];
      parsedata[fieldname].data = [];
      for (let i = strti; i <= endi; i++) {
        // let memberitem
        parsedata[fieldname].data[i] = {
          adr: { byte: adrob.byte, bit: adrob.bit },
        };
        parsesection(
          parsedata[fieldname].data[i],
          memberitem.Sections.Section,
          adrob,
        );
        // parsemember (parsedata, memberitem, adrob)
      }
...
function parsesingle(parsedata, memberitem, adrob) {
  const fieldname = memberitem._attributes.Name;
  let filedtype = memberitem._attributes.Datatype.toLowerCase().replace(
    /"/g,
    '',
  );
  parsedata[fieldname] = { type: filedtype };
  if (memberitem.StartValue) {
    // 07.12.21
    // console.log (memberitem.StartValue);
    const startvalob = memberitem.StartValue._text;
...
function parseudtall(types, udtpath) {
  const udtfiles = [];
  const filelist = fs.readdirSync(udtpath);
  for (filename of filelist) {
    // console.log (filename);
    if (
      filename.search('.udt') > 0
      && (filename.search('_STA') > 0
        || filename.search('_ALM') > 0
        || filename.search('PLC_ALM') > 0)
    ) {
      // console.log ('->' + filename);
...
function parsetypeudt(filename, plctypes) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (line.search('TITLE') >= 0) {
      // TITLE = Універсальний тип
      ob.structdescr = line.split('= ')[1];
      // console.log (ob.structdescr );
    }
    const l = line.search('//');
    if (l >= 0 && l < 3) {
      // //Цей тип використовується...
  ...
  const structend = 'END_STRUCT';
  const version = 'VERSION';
  let n = udtcontent.search(structstart);
  let m = udtcontent.search(structend);
  const structbody = udtcontent.substring(n + structstart.length, m);
  n = 0;
  m = udtcontent.search(version);
  ...
  let prevtype = '';
  for (line of lines) {
    const res = getAndReplacePlaceholders(line, '{ ', '}', ''); // замінити {} на пусті
    line = res.text;
    const ar = line.replace(':', '|').replace('//', '|').split('|');
    if (ar.length >= 2) {
      n = line.search('{');
      m = line.search('}');
      if (n > 1 && m > 1) line = line.slice(0, n) + line.slice(m + 1);
      const fieldname = ar[0].replace(/ /g, '');
