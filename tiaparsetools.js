...
function PLCCFG_to_plcs(dbcmplt, masterplcs) {
  ...
  if (parseplc.adr) {
    ...
    if (!masterplcs.memmap[dbcmplt.dbnum])
      masterplcs.memmap[dbcmplt.dbnum] = {};
...
function ACTHMI_to_acts(dbcmplt, masteracts) {
  ...
  for (actname in dbcmplt.data) {
    ...
    if (actplchmi.adr) {
      ...
      if (!masteracts.memmap[dbcmplt.dbnum])
        masteracts.memmap[dbcmplt.dbnum] = {};
      masteracts.memmap[dbcmplt.dbnum][startadrbyte] =
        "acts." + actname + ".plchmi";
    }
  }
}
...
function VARHMI_to_tags(dbcmplt, mastertags) {
  ...
  for (tagname in dbcmplt.data) {
    ...
    if (tagplchmi.adr) {
      ...
      if (!mastertags.memmap[dbcmplt.dbnum])
        mastertags.memmap[dbcmplt.dbnum] = {};
      mastertags.memmap[dbcmplt.dbnum][startadrbyte] =
        "tags." + tagname + ".plchmi";
    }
  }
}
...
function CHHMI_to_chs(parsedata, chs) {
  ...
  for (let i = 0; i < CHDIs.length; i++) {
    ...
    chhmi.type = "CH_HMI";
    chhmi.adr =
      "DB" + parsedata.dbnum + "." + CHDIs[i].adr.byte + "." + CHDIs[i].adr.bit;
  }
  for (let i = 0; i < CHDOs.length; i++) {
    ...
    chhmi.type = "CH_HMI";
    chhmi.adr =
      "DB" + parsedata.dbnum + "." + CHDOs[i].adr.byte + "." + CHDOs[i].adr.bit;
  }
  for (let i = 0; i < CHAIs.length; i++) {
    ...
    chhmi.type = "CH_HMI";
    chhmi.adr =
      "DB" + parsedata.dbnum + "." + CHAIs[i].adr.byte + "." + CHAIs[i].adr.bit;
  }
  for (let i = 0; i < CHAOs.length; i++) {
    ...
    chhmi.type = "CH_HMI";
    chhmi.adr =
      "DB" + parsedata.dbnum + "." + CHAOs[i].adr.byte + "." + CHAOs[i].adr.bit;
  }
}
...
function MODULES_to_chs(dbcmplt, masterchs) {
  ...
  for (let i = 0; i < dbcmplt.data.MODULES.data.length; i++) {
    ...
    if (module.adr) {
      if (!masterchs.memmap) masterchs.memmap = {};
      if (!masterchs.memmap[dbcmplt.dbnum])
        masterchs.memmap[dbcmplt.dbnum] = {};
...
function multilevel_adrparse(parsedata, dbnum) {
  ...
  if (parsedata.data && Array.isArray(parsedata.data) === true) {
    for (let fieldval of parsedata.data) {
      ...
      if (fieldval.adr) {
        if (startadrbyte < 0 || fieldval.adr.byte < startadrbyte)
          startadrbyte = fieldval.adr.byte;
    ...
    for (let fieldname in parsedata) {
      ...
      if (parsedata[fieldname].adr) {
        if (startadrbyte < 0 || parsedata[fieldname].adr.byte < startadrbyte)
          startadrbyte = parsedata[fieldname].adr.byte;
...
function parsedb(xmlcontent, parsedata) {
  let docs = xmlparser.xml2js(xmlcontent, {
    compact: true,
    spaces: 4,
  }).Document;
...
function parsemember(parsedata, memberitem, adrob) {
  ...
  if (memberitem.Sections) {
    ...
    if (memberitem.Comment)
      ob.descr = memberitem.Comment.MultiLanguageText._text;
...
function parsetypeudt(filename, plctypes) {
  ...
  //----------------
  let structstart = "STRUCT",
    structend = "END_STRUCT",
    version = "VERSION";
