...
function getmasterdataxls(masterdata) {
  const { masterdataxls } = masterdata.pathes;
  const wb = xlsx.readFile(masterdataxls);
  const wss = wb.Sheets;
  const wstags = wss.tags;
  const wsdraws = wss.draws;
  const wsacttps = wss.acttps;
  const wsDB = wss.DB;
  const wsiot = wss.iot;
  const wsCLSID = wss.CLSID;

  if (!masterdata.draws) masterdata.draws = { fsa: {}, circuit: {} };
  const { draws } = masterdata; // перелік схем привязаних до DEV
  const acttrtypes = masterdata.acttrtypes = {};
  const DBs = masterdata.DBs = xlsx.utils.sheet_to_json(wsDB);
  const iot = masterdata.iot = xlsx.utils.sheet_to_json(wsiot);
  ...
  for (row of xlsx.utils.sheet_to_json(wsacttps)) {
    const acttrtype = acttrtypes[row.ACTTYPE] = {
      CLSID: `16#${row.CLSID}`, fnname: row.FN, typename: row.ACTTYPE, altertypename: row.ALTERNAME, typedescr: row.ACTTYPEDESCR, io: {},
    };
    for (colname in row) {
  ...
  for (tag of tags) {
    ...
    const module = moduls[tag.MODID];

    if (!tag.SUBTYPE || tag.SUBTYPE.length < 2) {
      tag.SUBTYPE = 'deflt';
    }
    const chclass = masterdata.class[tag.TYPE][tag.SUBTYPE];
    ...
    const clsidfromtype = {
      DI: 0x1010, AI: 0x1030, DO: 0x1020, AO: 0x1040,
    };
  ...
  for (modulename in moduls) {
    ...
    module.chdi.sort((a, b) => a.CHID - b.CHID);
    module.chai.sort((a, b) => a.CHID - b.CHID);
    module.chdo.sort((a, b) => a.CHID - b.CHID);
    module.chao.sort((a, b) => a.CHID - b.CHID);
    // console.log (modulscnt + ' ' + modulename);
  }
  masterdata.iostatistic = {
    dicnt, aicnt, docnt, aocnt, modulscnt,
  };
...
function savemasterdatatofile(masterdata) {
  const filename = `${masterdata.pathes.masterpath + path.basename(masterdata.pathes.masterdataxls, '.xlsx')}.json`;
...
function iomaptoplcform(masterdata) {
  const submodtypes = {
    0: '-', 1: 'DI', 2: 'DO', 3: 'AI', 4: 'AO',
  };// 1- DICH, 2- DOCH, 3- AICH, 4 – AOCH
  const { moduls } = masterdata;
  ...
  for (modulename in moduls) {
    ...
    for (let i = 0; i < modulegenform.submdicnt; i++) {
      modulegenform.submodules[nmbsubmodule] = {
        type: '1', chidstart: module.chdi[i * 16].CHID, adrstart: module.chdi[i * 16].SRCADR, chcnt: i + 1 < module.submdicnt ? 16 : (module.chdi.length === 16) ? 16 : (module.chdi.length % 16),
      };
      nmbsubmodule++;
    }
    for (let i = 0; i < modulegenform.submdocnt; i++) {
      modulegenform.submodules[nmbsubmodule] = {
        type: '2', chidstart: module.chdo[i * 16].CHID, adrstart: module.chdo[i * 16].SRCADR, chcnt: i + 1 < module.submdocnt ? 16 : (module.chdo.length === 16) ? 16 : (module.chdo.length % 16),
      };
      nmbsubmodule++;
    }
    for (let i = 0; i < modulegenform.submaicnt; i++) {
      modulegenform.submodules[nmbsubmodule] = {
        type: '3', chidstart: module.chai[i * 16].CHID, adrstart: module.chai[i * 16].SRCADR, chcnt: i + 1 < module.submaicnt ? 16 : (module.chai.length === 16) ? 16 : (module.chai.length % 16),
      };
      nmbsubmodule++;
    }
    for (let i = 0; i < modulegenform.submaocnt; i++) {
      modulegenform.submodules[nmbsubmodule] = {
        type: '4', chidstart: module.chao[i * 16].CHID, adrstart: module.chao[i * 16].SRCADR, chcnt: i + 1 < module.submaocnt ? 16 : (module.chao.length === 16) ? 16 : (module.chao.length % 16),
      };
      nmbsubmodule++;
    }
...
function iomaptoexcel(masterdata) {
  const { masterpath } = masterdata.pathes;
  ...
  const wsiomap = xlsx.utils.json_to_sheet(iomap.plcform);
...
function getactrtinfo(masterdata) {
  ...
  for (acttrname in acttrprops) {
    const acttrprop = acttrprops[acttrname];
  ...
  let actlistlog = '';
  for (actrname in acttrprops) {
    const actprop = acttrprops[actrname];
...
function attrlinktag(acttrprop, masterdata) {
  const typename = acttrprop.type;
  const acttype = masterdata.acttrtypes[typename];
  if (typeof acttype === 'undefined') return;
  acttrprop.io = { unlinked: [] };
  for (const ioname in acttype.io) {
    acttrprop.io[ioname] = '';
  }
  const acttps = masterdata.acttrtypes;
  for (tag of acttrprop.tags) {
    ...
    for (ioname in acttrprop.io) {
      const ioar = ioname.split('/');
      for (const io of ioar) {
        if (io === sufix) {
          found = true;
...
module.exports = {
  getmasterdataxls, iomaptoplcform, iomaptoexcel, getactrtinfo, savemasterdatatofile, readmasterfromjson, getdatafromspec,
};
