...
const masterpath = envvar.masterpath;
...
function create_plcmapsscl (masterdata) {
  ...
  let header = `FUNCTION "PLCMAPS" : Void
  ...
  let filename = envvar.codepath + 'Program blocks/PLC/PLCMAPS.scl';
...
function create_pstsdb (masterdata) {
  ...
  `
  let filename = envvar.codepath + 'Program blocks/PLC/SYS.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  } 
  fs.writeFileSync(filename, '\ufeff' + filecontent.replace(/\x0A/g, '\x0D\x0A'), 'utf8');
...
function create_chdb (masterdata) {
  filecontent = `DATA_BLOCK "CH"
    ...
    END_DATA_BLOCK
    `
  let filename = envvar.codepath + 'Program blocks/CH/CH.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  }   
...
function create_dbmodulesdb (masterdata) {
  ...
  filecontent =`
    ...
    END_DATA_BLOCK
  `
  let filename = envvar.codepath + 'Program blocks/PLC/DBMODULES.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  }   
  fs.writeFileSync(filename, '\ufeff' + filecontent.replace(/\x0A/g, '\x0D\x0A'), 'utf8');
...
function create_vardb (masterdata) {
  ...
  for (tag of tags) {
    ...
    if (rforeign.test(tag.TAGNAME)) {
      ...
      `;      
      let chid =  tag.TAGNAME.substr(0, 3).toLowerCase()==='rez' ? 0 : tag.CHID; 
...
END_DATA_BLOCK
` 
  let filename = envvar.codepath + 'Program blocks/VAR/VAR.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  }  
  fs.writeFileSync(filename, '\ufeff' + filecontent.replace(/\x0A/g, '\x0D\x0A'), 'utf8');
...
` 
  ...
  let acttrsbytypes = JSON.parse (JSON.stringify(masterdata.acttrtypes));
