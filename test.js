...
const cryptojs = require('crypto-js'); //https://github.com/brix/crypto-js  
...
function test_tagsdif () {
  let plcfilemaster = tiaresultfiles + 'plc_tags.json';
  let cfgfilemaster = tiaresultfiles + 'cfg_tags.json';
  masterdatatools.opts.logfile = 'test_tagsdif.log';
  let content = fs.readFileSync (plcfilemaster,'utf8');
  let plctags = JSON.parse (content);
  content = fs.readFileSync (cfgfilemaster,'utf8');
  let cfgtags = JSON.parse (content); 
  let difob = masterdatatools.tagsdif(plctags, cfgtags);
...
function testcfgtools () {
  ...
  let cfgtags = test_getcfgtags_fromxls ();
  let cfgacts = test_getcfacts(cfgtags);

  let cfgchs = {};
  let cfgchmap = test_chsmap_fromcfg (cfgchs, cfgtags);
...
function test_vars_totags () {
  ...
  let listfiles =  {udtfiles : config.tiaparsetools.var_udtfiles.replace(/ /g, '').split(','),
...
function test_chsmap_fromplc (plchs, plctags){
  ...
  let chsmap = masterdatatools.chsmap_fromplc (plchs, plctags);
