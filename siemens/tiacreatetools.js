...
function create_plcmapsscl (cfgchs) {
  ...
  let filename = opts.resultpath + '/PLCMAPS.scl';
...
function create_chdb (cfgchs) {
  filecontent = `DATA_BLOCK "CH"
    ...
    END_DATA_BLOCK
    `
  let filename = opts.resultpath + '/CH.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  }   
...
function create_dbmodulesdb (cfgchs) {
  ...
  filecontent =`
    ...
    END_DATA_BLOCK
  `
  let filename = opts.resultpath + '/DBMODULES.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  }   
  fs.writeFileSync(filename, '\ufeff' + filecontent.replace(/\x0A/g, '\x0D\x0A'), 'utf8');
...
END_DATA_BLOCK
` 
  let filename = opts.resultpath + '/VAR.db';
  if (!fs.existsSync(path.dirname(filename))) {
    fs.mkdirSync(path.dirname(filename))
  }  
  fs.writeFileSync(filename, '\ufeff' + filecontent.replace(/\x0A/g, '\x0D\x0A'), 'utf8');
